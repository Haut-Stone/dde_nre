...
parser.add_argument('--dataset', default='none', choices=['none', 'semeval', 'wiki80', 'tacred'],  # 是否有已经有的数据集
...
# Hyper-parameters
parser.add_argument('--batch_size', default=160, type=int,  # 批大小
        help='Batch size')
parser.add_argument('--lr', default=1e-1, type=float,  # 学习率
        help='Learning rate')
parser.add_argument('--weight_decay', default=1e-5, type=float,  # 权重衰减
        help='Weight decay')
parser.add_argument('--max_length', default=128, type=int,  # 最大的句子长度，根据数据集确定
        help='Maximum sentence length')
parser.add_argument('--max_epoch', default=100, type=int,  # 最大的训练轮数
...
if args.dataset != 'none':  # 如果是已经有现存的数据集，那么要下载数据集
    ...
    args.train_file = os.path.join(root_path, 'benchmark', args.dataset, '{}_train.txt'.format(args.dataset))  # 载入数据集
...
# Download glove
opennre.download('glove', root_path=root_path)  # 下载glove 词向量
word2id = json.load(open(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_word2id.json')))  # 词和id的转换
word2vec = np.load(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_mat.npy'))  # 词向量
