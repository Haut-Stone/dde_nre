...
parser.add_argument('--dataset', default='none', choices=['none', 'wiki_distant', 'nyt10', 'nyt10m', 'wiki20m', 'dde'],
...
if not os.path.exists('ckpt'):  # 如果没有文件夹则建立这个文件夹
    os.mkdir('ckpt')
if len(args.ckpt) == 0:
    args.ckpt = '{}_{}'.format(args.dataset, 'pcnn_att')  # 生成ckpt文件以 数据集名_pcnn_att 命名
ckpt = 'ckpt/{}.pth.tar'.format(args.ckpt)

if args.dataset != 'none':
    args.train_file = os.path.join(root_path, 'data', args.dataset, '{}_train.txt'.format(args.dataset))
    args.val_file = os.path.join(root_path, 'data', args.dataset, '{}_val.txt'.format(args.dataset))
    if not os.path.exists(args.val_file):
        logging.info("Cannot find the validation file. Use the test file instead.")  # 如果没有验证集则用测试集代替
        args.val_file = os.path.join(root_path, 'data', args.dataset, '{}_test.txt'.format(args.dataset))
    args.test_file = os.path.join(root_path, 'data', args.dataset, '{}_test.txt'.format(args.dataset))
    args.rel2id_file = os.path.join(root_path, 'data', args.dataset, '{}_rel2id.json'.format(args.dataset))
else:
