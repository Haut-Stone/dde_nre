...
def download_pretrain(model_name, root_path=default_root_path):
    ckpt = os.path.join(root_path, 'pretrain/nre/' + model_name + '.pth.tar')  # 这个莫非是针对 wiki80 已经训练好的数据集，这个是一个我们需要训练的东西。
...
def get_model(model_name, root_path=default_root_path):
    ...
    if model_name == 'wiki80_cnn_softmax':
        ...
        download('glove', root_path=root_path)  # 训练用的词向量
        download('wiki80', root_path=root_path)  # 训练用的数据集
        wordi2d = json.load(open(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_word2id.json')))  # 获得词和id的对应关系
        word2vec = np.load(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_mat.npy'))  # 获得训练好的词向量
        rel2id = json.load(open(os.path.join(root_path, 'benchmark/wiki80/wiki80_rel2id.json')))  # 关系到 id 的转换关系，这个估计我们训练时可以自己定义
        sentence_encoder = encoder.CNNEncoder(token2id=wordi2d,  # 设置解码器的一些参数
                                                     max_length=40,
                                                     word_size=50,
                                                     position_size=5,
                                                     hidden_size=230,
                                                     ...
                                                     word2vec=word2vec,
                                                     dropout=0.5)
        m = model.SoftmaxNN(sentence_encoder, len(rel2id), rel2id)  # 设置分类器
        m.load_state_dict(torch.load(ckpt, map_location='cpu')['state_dict'])  # 使用 torch 载入模型
        return m
        ...
        if 'entity' in model_name:
            sentence_encoder = encoder.BERTEntityEncoder(
                max_length=80, pretrain_path=os.path.join(root_path, 'pretrain/bert-base-uncased'))  # 如果是使用 bert 的话则是直接使用 bert
        else:
        ...
        m.load_state_dict(torch.load(ckpt, map_location='cpu')['state_dict'])
        return m
    elif model_name == 'dde_cnn':
        check_root()
        ckpt = os.path.join(root_path, 'pretrain/nre/' + model_name + '.pth.tar')
        word2id = json.load(open(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_word2id.json')))  # 获得词和id的对应关系
        word2vec = np.load(os.path.join(root_path, 'pretrain/glove/glove.6B.50d_mat.npy'))  # 获得训练好的词向量
        rel2id = json.load(open(os.path.join(root_path, '../data/dde/dde_rel2id.json')))  # 关系到 id 的转换关系，这个估计我们训练时可以自己定义
